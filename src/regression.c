...
#define IN_1MATRIX_OUT_MATRIX_IN    "matrix",           "matrix",               "matrix"
#define IN_1MATRIX_OUT_MATRIX_OUT   "matrix_transp",    "matrix_inv",           "matrix_inv"
...
#define IN_2MATRIX_OUT_MATRIX_IN0   "matrix",       "matrix",           "matrix",               "matrix",                 
#define IN_2MATRIX_OUT_MATRIX_IN1   "matrix",       "matrix",           "B",                    "B",                      
#define IN_2MATRIX_OUT_MATRIX_OUT   "matrix_add",   "matrix_mult",      "X",                    "X",                      
#define IN_2MATRIX_OUT_MATRIX_NAME  "matrix_add_f", "matrix_mult_f",    "matrix_solve_plu_f",   "matrix_solve_cholesky_f",
...
int test_matrix_equality(const matrix_t *matrix1, const matrix_t *matrix2){
    ...
    for (int i=0; i<matrix1->rows; i++){
        for (int j=0; j<matrix1->columns; j++){
            if(fabs(creal(matrix1->coeff[i][j] - matrix2->coeff[i][j])) > 0.000001)return 0;
            if(fabs(cimag(matrix1->coeff[i][j] - matrix2->coeff[i][j])) > 0.000001)return 0;
        }
    }
    return 1;
}
...
result_t test_function_in_matrix_out_double_f(double complex(*function)(const matrix_t*), matrix_t* matrix, double complex expected, char *test_name)
{
    ...
    long long time2 = utime();
    fabs(-1+creal(ret)/creal(expected));
    result_t res = {test_name, fabs(-1+creal(ret)/creal(expected))<0.00001, time2 - time};
    return(res);
}
...
int main(int argc, char **argv) {
    char *data_path;
    if(argc <2)
        data_path = "/home/Drakorcarnis/new_library/test/data";
    else
        data_path = argv[1];
    ssize_t size;
    
    double complex(*test_function_in_1matrix_out_double_f[])(const matrix_t*) = {IN_1MATRIX_OUT_DOUBLE};
    char* test_function_in_1matrix_out_double_in_name[] = {IN_1MATRIX_OUT_DOUBLE_NAME};
    
    char* test_function_in_1matrix_out_double_in[] = {IN_1MATRIX_OUT_DOUBLE_IN};
    matrix_t** test_function_in_1matrix_out_double_in_input = NULL;
    size = sizeof(test_function_in_1matrix_out_double_in)/sizeof(char*);
    test_function_in_1matrix_out_double_in_input = chartab2matrixtab(test_function_in_1matrix_out_double_in, size, data_path);
    
    double complex test_function_in_1matrix_out_double_out[] = {IN_1MATRIX_OUT_DOUBLE_OUT};
    // matrix_t** test_function_in_1matrix_out_double_in_output = NULL;
    // size = sizeof(test_function_in_1matrix_out_double_out)/sizeof(char*);
    // chartab2matrixtab(test_function_in_1matrix_out_double_out, test_function_in_1matrix_out_double_in_output, size);
    
    for (int i = 0; i < size; i++)
        process_result(test_function_in_matrix_out_double_f(test_function_in_1matrix_out_double_f[i],test_function_in_1matrix_out_double_in_input[i], test_function_in_1matrix_out_double_out[i], test_function_in_1matrix_out_double_in_name[i]));
    
    free_matrixtab(test_function_in_1matrix_out_double_in_input, size);
    // free_matrixtab(test_function_in_1matrix_out_double_in_output, size);
    
    // matrix_t*(*test_function_in_1matrix_out_matrix_f[])(const matrix_t*) = {IN_1MATRIX_OUT_MATRIX};
    // matrix_t*(*test_function_in_2matrix_out_matrix_f[])(const matrix_t*, const matrix_t*) = {IN_2MATRIX_OUT_MATRIX};
